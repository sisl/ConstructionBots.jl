...
# ldraw_file                   = "rear_wheel_and_tire.ldr"
# project_name                 = "rear_wheel_and_tire"
# num_robots                   = 2
...
plt = GraphPlottingBFS.display_graph(
    sched;
    grow_mode=:from_left,
    ...
    draw_node_function=(G,v)->GraphPlottingBFS.draw_node(get_node(G,v);
        title_text= _node_type_check(get_node(G,v)
            ) ? string(GraphPlottingBFS._title_string(get_node(G,v)),
                "$(get_id(node_id(get_node(G,v))))") : GraphPlottingBFS._title_string(get_node(G,v)),
        subtitle_text="",
