...
ConstructionBots.compute_approximate_geometries!(
    scene_tree,
    ConstructionBots.HypersphereKey(),
)
ConstructionBots.compute_approximate_geometries!(
    scene_tree,
    ConstructionBots.HyperrectangleKey(),
)
...
root = ConstructionBots.get_node(
    scene_tree,
    collect(ConstructionBots.get_all_root_nodes(scene_tree))[1],
)
ConstructionBots.validate_tree(ConstructionBots.get_transform_node(root))
ConstructionBots.validate_embedded_tree(
    scene_tree,
...
@assert all(
    map(
        node -> ConstructionBots.has_vertex(
            node.geom_hierarchy,
            ConstructionBots.HypersphereKey(),
        ),
        ConstructionBots.get_nodes(scene_tree),
    ),
)
ConstructionBots.compute_approximate_geometries!(
    scene_tree,
...
sched = ConstructionBots.construct_partial_construction_schedule(
    model,
    model_spec,
    scene_tree,
    id_map,
...
plt = ConstructionBots.display_graph(
    ...
    draw_node_function = (G, v) -> ConstructionBots.draw_node(
        ConstructionBots.get_node(G, v);
        title_text = _node_type_check(ConstructionBots.get_node(G, v)) ?
                     string(
            ConstructionBots._title_string(ConstructionBots.get_node(G, v)),
            "$(ConstructionBots.get_id(ConstructionBots.node_id(ConstructionBots.get_node(G,v))))",
        ) : ConstructionBots._title_string(ConstructionBots.get_node(G, v)),
        subtitle_text = "",
