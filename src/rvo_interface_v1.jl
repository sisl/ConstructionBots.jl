...
# RVOState initializer and setter methods
function rvo_reset_agent_map!(state::RVOState)
    state.id_map = RVOAgentMap()
end

rvo_active_agents(scene_tree) = (get_node(scene_tree, node_id(n)) for n in get_nodes(rvo_global_id_map()))

function reset_rvo_python_module!(state::RVOState)
    state.python_module = nothing
    state.python_module = pyimport("rvo2")
end

# RVO simulation methods
function rvo_new_sim(state::RVOState; dt::Float64=1 / 40.0, neighbor_dist::Float64=2.0,
...
function rvo_set_new_sim!(state::RVOState, sim=nothing)
...
# RVOAgent methods
function rvo_add_agent!(state::RVOState, agent::Union{RobotNode, TransportUnitNode}, sim)
...
function rvo_get_agent_position(state::RVOState, n)
    rvo_idx = rvo_get_agent_idx(state, n)
    get_element(state.sim_wrapper).getAgentPosition(rvo_idx)
end

function rvo_set_agent_position!(state::RVOState, node, pos)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).setAgentPosition(idx, (pos[1], pos[2]))
end

function rvo_set_agent_pref_velocity!(state::RVOState, node, vel)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).setAgentPrefVelocity(idx, (vel[1], vel[2]))
end

function rvo_get_agent_pref_velocity(state::RVOState, node)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).getAgentPrefVelocity(idx)
end

function rvo_get_agent_velocity(state::RVOState, node)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).getAgentVelocity(idx)
end

function rvo_set_agent_max_speed!(state::RVOState, node, speed=nothing)
    speed = isnothing(speed) ? rvo_default_max_speed(state) : speed
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).setAgentMaxSpeed(idx, speed)
end

function rvo_set_agent_alpha!(state::RVOState, node, alpha=0.5)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).setAgentAlpha(idx, alpha)
end

function rvo_get_agent_alpha(state::RVOState, node)
    idx = rvo_get_agent_idx(state, node)
    get_element(state.sim_wrapper).getAgentAlpha(idx)
end

# RVO parameter methods
function rvo_default_max_speed(state::RVOState)
    state.max_speed
end

function set_rvo_default_max_speed!(state::RVOState, val)
    state.max_speed = val
end

function rvo_default_max_speed_volume_factor(state::RVOState)
    state.max_speed_volume_factor
end

function set_rvo_default_max_speed_volume_factor!(state::RVOState, val)
    state.max_speed_volume_factor = val
end

function rvo_default_min_max_speed(state::RVOState)
    state.min_max_speed
end

function set_rvo_default_min_max_speed!(state::RVOState, val)
    state.min_max_speed = val
end

function rvo_default_time_step(state::RVOState)
    state.default_time_step
end

function set_rvo_default_time_step!(state::RVOState, val)
    state.default_time_step = val
end

function rvo_default_neighbor_distance(state::RVOState)
    state.default_neighbor_distance
end

function set_rvo_default_neighbor_distance!(state::RVOState, val)
    state.default_neighbor_distance = val
end

function rvo_default_min_neighbor_distance(state::RVOState)
    state.default_min_neighbor_distance
end

function set_rvo_default_min_neighbor_distance!(state::RVOState, val)
    state.default_min_neighbor_distance = val
end

function rvo_default_neighborhood_velocity_scale_factor(state::RVOState)
    state.default_neighborhood_velocity_scale_factor
end

function set_rvo_default_neighborhood_velocity_scale_factor!(state::RVOState, val)
...
"""
function get_rvo_neighbor_distance(state::RVOState, node)
...
function not_already_mapped(state::RVOState, node)
    !has_vertex(state.id_map.graph, node_id(node))
end

# Agent addition and simulation update
function rvo_add_agents!(state::RVOState, scene_tree)
...
function rvo_sim_needs_update(state::RVOState, scene_tree)
    for node in get_nodes(scene_tree)
