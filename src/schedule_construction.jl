...
for T in (:OpenBuildStep, :CloseBuildStep)
    @eval begin
        $T(a::AssemblyNode, c::Dict, g1::GeomNode, g2::GeomNode) =
            $T(a, c, g1, g2, get_id(get_unique_id(TemplatedID{$T})))
        $T(a::AssemblyNode, c::Dict) = $T(
            a,
            c,
        ...
        $T(n::BuildPhasePredicate) =
            $T(n.assembly, n.components, n.staging_circle, n.bounding_circle, n.id)
    end
end
...
cargo_node_type(n::TransportUnitNode) =
    cargo_type(n) == AssemblyNode ? AssemblyComplete : ObjectStart
...
required_predecessors(n::FormTransportUnit) =
    Dict(RobotGo => length(robot_team(n)), cargo_node_type(n) => 1)
...
required_successors(n::DepositCargo) =
    Dict(LiftIntoPlace => 1, RobotGo => length(robot_team(n)))
...
required_successors(::AssemblyComplete) =
    Dict(Union{FormTransportUnit,ProjectComplete} => 1)
...
"""
function get_previous_build_step(
    model_spec,
    v;
    skip_first = matches_template(SubModelPlan, get_node(model_spec, v)),
)
    vp = depth_first_search(
        model_spec,
        get_vtx(model_spec, v),
        u -> matches_template(BuildingStep, get_node(model_spec, u)),
        u -> (
            get_vtx(model_spec, v) == u ||
            !matches_template(SubModelPlan, get_node(model_spec, u))
        ),
        inneighbors;
...
- id_map
"""
function populate_schedule_build_step!(
    sched,
    ...
    for child_id in get_build_step_components(model_spec, id_map, step_node)
        ...
        set_local_transform!(
            goal_config(l),
            child_transform(entity(parent), node_id(cargo)),
        )
        ...
        set_desired_global_transform!(
            ...
            CoordinateTransformations.Translation(
                global_transform(start_config(l)).translation,
            ),
        )
...
"""
function populate_schedule_sub_graph!(
    sched,
    parent::AssemblyComplete,
    model_spec,
    scene_tree,
    ...
    while !(step_node === nothing)
        ...
        ob = populate_schedule_build_step!(
            sched,
            parent,
            cb,
            step_node,
        ...
        for n in get_nodes(sched)
            if matches_template(OpenBuildStep, n)
                ...
                for v in outneighbors(sched, n)
                    ...
                    assert_transform_tree_ancestor(
                        goal_config(deposit_node),
                        start_config(assembly_complete),
                    )
                    for vp in outneighbors(sched, v)
                        lift_node = get_node(sched, vp)
                        if matches_template(LiftIntoPlace, lift_node)
                            @assert has_child(
                                goal_config(assembly_complete),
                                goal_config(lift_node),
                            )
                            if post_staging
            ...
            elseif matches_template(Union{DepositCargo,FormTransportUnit}, n)
                ...
                if post_staging
                    @assert isapprox(
                        global_transform(goal_config(n)).translation[3],
                        0.0;
                        rtol = 1e-6,
                        atol = 1e-6,
                    ) "$n, $(global_transform(goal_config(n)).translation)"
                end
            ...
            elseif matches_template(LiftIntoPlace, n)
                ...
                if post_staging
                    ...
                    @assert isapprox(
                        global_transform(start_config(f)).translation[3],
                        0.0;
                        rtol = 1e-6,
                        atol = 1e-6,
                    ) "Z translation should be 0 for $(summary(node_id(f)))"
                    @assert isapprox(
                        global_transform(start_config(d)).translation[3],
                        0.0;
                        rtol = 1e-6,
                        atol = 1e-6,
                    ) "Z translation should be 0 for $(summary(node_id(d)))"
...
"""
function generate_staging_plan!(
    scene_tree,
    sched;
    buffer_radius = 0.0,
    ...
    if !all(map(n -> has_vertex(n.geom_hierarchy, HypersphereKey()), get_nodes(scene_tree)))
        compute_approximate_geometries!(scene_tree, HypersphereKey(); ϵ = 0.0)
    end
    ...
    select_assembly_start_configs_layered!(
        sched,
        scene_tree,
        staging_circles;
...
locations.
"""
function select_assembly_start_configs_layered!(
    sched,
    scene_tree,
    staging_circles;
    buffer_radius = 0.0,
    ...
    for start_node in node_iterator(sched, topological_sort_by_dfs(sched))
        ...
        if matches_template(AssemblyNode, node)
            ...
            assembly = node
            part_ids = sort(
                filter(k -> isa(k, AssemblyID), collect(keys(assembly_components(node)))),
            )
            if isempty(part_ids)
                continue
            end
            ...
            while !isempty(part_ids_left)
                ...
                radii_left =
                    [staging_circles[id].radius + buffer_radius for id in part_ids_left]
                ...
                elseif can_fit_steps == maximum(unique_step_lengths)
                    ...
                    arc_dist_used =
                        sum(asin.(radii_for_opt ./ (radii_for_opt .+ staging_radius)))
                    ...
                    half_widths = asin.(sorted_radii ./ (sorted_radii .+ staging_radius))
                    last_idx = findlast(
                        (arc_dist_used .+ cumsum(half_widths)) * 2 .<= 2.0 * Float64(π),
                    )
                ...
                parts_for_opt =
                    (get_node(scene_tree, part_id) for part_id in part_ids_for_opt)
                radii_for_opt =
                    [staging_circles[id].radius + buffer_radius for id in part_ids_for_opt]
                ...
                while staging_radius - ring_radius > 1e-6
                    ...
                    for (part_id, part) in zip(part_ids_for_opt, parts_for_opt)
                        ...
                        d_ = project_to_2d(tform.translation) - staging_circle.center  # the vector from the circle center to the goal location
                    ...
                    θ_star, staging_radius =
                        solve_ring_placement_problem(θ_des, radii_for_opt, ring_radius)
                end
                # Compute staging config transforms (relative to parent assembly)
                tformed_geoms = Vector{LazySets.Ball2}()
                for (i, (θ, r, part_id, part)) in
                    enumerate(zip(θ_star, radii_for_opt, part_ids_for_opt, parts_for_opt))
                ...
                @assert new_ball.radius + 1e-5 >
                        old_ball.radius + norm(new_ball.center .- old_ball.center) "$(summary(node_id(assembly))) old ball $(old_ball), new_ball $(new_ball)"
...
locations.
"""
function select_assembly_start_configs!(
    sched,
    scene_tree,
    staging_circles;
    buffer_radius = 0.0,
    ...
    for start_node in node_iterator(sched, topological_sort_by_dfs(sched))
        ...
        if matches_template(AssemblyNode, node)
            ...
            while staging_radius - ring_radius > 1e-6
                ...
                # Optimize placement and increase staging_radius if necessary
                θ_star, staging_radius =
                    solve_ring_placement_problem(θ_des, radii, ring_radius)
            end
            # Compute staging config transforms (relative to parent assembly)
            tformed_geoms = Vector{LazySets.Ball2}()
...
"""
function process_schedule_build_step!(
    node,
    sched,
    scene_tree,
    bounding_circles,
    ...
    bounding_circle = get!(
        bounding_circles,
        node_id(assembly),
        LazySets.Ball2(zeros(SVector{2,Float64}), 0.0),
    ...
    if length(geoms) == 1 && bounding_circle.radius < 1e-6
        ...
        θ_star, assembly_radius =
            solve_ring_placement_problem(θ_des, radii, bounding_circle.radius)
        Δθ = θ_star .- θ_des
    ...
    # Compute staging config transforms (relative to parent assembly)
    tformed_geoms = Vector{LazySets.Ball2}()
    for (i, (θ, r, part_id, part, tform)) in
        enumerate(zip(θ_star, radii, part_ids, parts, tforms))
        tu = get_node(scene_tree, TransportUnitNode(part))
    ...
    open_build_step.bounding_circle.base_geom =
        project_to_3d(bounding_circles[node_id(node)])
    open_build_step.staging_circle.base_geom =
        project_to_3d(staging_circles[node_id(assembly)])
...
return θ_star
"""
function solve_ring_placement_problem(
    θ_des,
    r,
    R,
    rmin = 0.0;
    ...
    @constraint(model, θ[n] + half_widths[n] <= θ[1] - half_widths[1] + 2.0 * Float64(π))  # wrap-around constraint
    ...
    for node in get_nodes(sched)
        if matches_template(AssemblyComplete, node)
            ...
            for (id, tf) in assembly_components(assembly)
                ...
                start_node, form_transport, go_node, deposit_node, lift_node =
                    get_transport_node_sequence(sched, cargo)
...
"""
function align_construction_predicates!(
    sched,
    start::Union{ObjectStart,AssemblyComplete,LiftIntoPlace},
    t::Union{DepositCargo,FormTransportUnit},
)
    ...
    delta_z =
        tform.translation[3] - global_transform(cargo_deployed_config(t)).translation[3]
...
align_construction_predicates!(sched, a::CustomNode, b::CustomNode) =
    align_construction_predicates!(sched, node_val(a), node_val(b))
...
get_parent_build_step(sched, n::Union{FormTransportUnit,TransportUnitGo}) =
    get_parent_build_step(sched, get_node(sched, DepositCargo(entity(n))))
...
get_first_build_step(sched, n::AssemblyStart) =
    get_node(sched, first(outneighbors(sched, n)))
...
"""
function set_scene_tree_to_initial_condition!(scene_tree, sched; remove_all_edges = false)
    if remove_all_edges
...
function select_initial_object_grid_locations!(sched, vtxs)
    ...
    tforms = map(
        v ->
            CoordinateTransformations.Translation(v[1], v[2], v[3]) ∘ identity_linear_map(),
        vtxs,
    )
...
"""
function add_robots_to_scene!(
    scene_tree,
    vtxs,
    geoms = (default_robot_geom() for v in vtxs),
)
    for (vtx, geom) in zip(vtxs, Base.Iterators.cycle(geoms))
        tform =
            CoordinateTransformations.Translation(vtx[1], vtx[2], 0.0) ∘
            identity_linear_map()
        robot_node =
            add_node!(scene_tree, RobotNode(get_unique_id(RobotID), GeomNode(geom)))
...
"""
function configure_transport_unit(
    cargo,
    pts;
    ...
    cargo_tform =
        CoordinateTransformations.Translation(0.0, 0.0, robot_height - zmin) ∘
        identity_linear_map()
    ...
    for pt in support_pts
        tform =
            CoordinateTransformations.Translation(pt[1], pt[2], 0.0) ∘ identity_linear_map()
...
`transport_unit`.
"""
function add_temporary_invalid_robots!(
    scene_tree,
    transport_unit;
...
function find_step_numbers(start_node, part_ids, sched, scene_tree; max_depth = 1000)
    ...
    while continue_looking
        ...
        if length(ns) < 1 || cnt > max_depth
            ...
            error(
                "Could not find node $(lift_nodes_to_find[idxs]) in $(start_node.id). Searched to depth $(cnt)",
            )
        end
    end
    return steps_found
